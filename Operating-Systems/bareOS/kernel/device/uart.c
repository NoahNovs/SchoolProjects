...
void uart_handler(void) {
  ...
  if (code == UART_RX_INTR) {
    ...
    tty_in[tty_in_head] = readchar;
    tty_in_count++;
  }
    

  else if (code == UART_TX_INTR) {
    if (tty_out_count > 0) {
      char c = tty_out[tty_out_head];  //  Function MUST get a character from tty_out

      // and place it in the uart (see Appendix)
      //uart_putc(c);  // Send character via UART
      ...
      uart[UART0_RW_REG] = c;
      if (tty_out_count == 0) {
        set_uart_interrupt(0);  // Disable UART interrupts if tty_out buffer is empty
      }
    }
  }
